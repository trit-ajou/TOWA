...
- inpaint provider 호출이 실제 mask guide를 받도록 수정했다. `mask_or_erase_planning`의 per-task erase mask를 full-page black/white guide image로 합성하고, Mindlogic/Nanobanana edit 호출에 원본 페이지와 guide image를 함께 전달한다. Mindlogic Google edit payload는 Mindlogic 문서의 flat `reference_images[].image_bytes/mime_type` 스키마로 `REFERENCE_TYPE_MASK`, `EDIT_MODE_INPAINT_REMOVAL`, `mask_dilation=0.01`을 보낸다. task prompt의 “image 2 white areas” 지시가 실제 입력과 일치하며, stage report에는 `provider_reference_image_count`, `provider_mask_guide`를 남기고 debug artifact `provider_mask_guide_bitmap`도 저장한다.
