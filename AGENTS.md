...
- `python` 대신 `python3`를 사용하세요.
- plan 모드가 아니더라도, 명확한 수정 지시가 없으면 파일을 수정하지 말고 사용자에게 허락을 구하세요. 단, 간단한 수정은 즉시 패치해도 괜찮습니다.
- 변경이 논리적으로 완결되고 체크포인트로 남길 가치가 있으면 적극적으로 커밋하세요.
- `AGENTS.md` 수정이 필요한 경우, 직접 수정하지 말고 사용자에게 요청하세요. 단, `Docs` 세션의 문서 목록은 수정하셔도 됩니다.
- fast-forward merge 금지합니다.
...
### 오류 발생 시 문서 작성 요령
- 검증 중 오류나 예외가 발생했을 때 아래 절차에 따릅니다:
  1. 현재 진행 중인 task overall을 문서화하여 `임시 계획 문서`를 작성합니다. 이미 `임시 계획 문서`가 존재한다면 생략합니다.
  2. 라이브 검증, probe, 로그 분석, 코드 분석 등을 통해 원인을 파악합니다. 필요하다면 코드를 수정해도 됩니다.
  3. 해당 오류에 대해 `임시 오류 문서`를 작성합니다.
  4. 패치 전 워크플로우의 전체적인 철학과 AGENTS.md의 방향성을 먼저 파악하세요. 이를 `임시 오류 문서`에 업데이트합니다.
  5. 워크플로우의 전체적인 철학과 AGENTS.md의 방향성을 크게 해치지 않는 선에서 수정 계획을 세웁니다. 수정 계획을 `임시 오류 문서`에 업데이트합니다.
