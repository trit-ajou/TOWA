...
## bitmappery 캔버스 활용 방침

- **편집 화면(역자 모드)과 상세 편집(식자 모드) 모두 bitmappery 캔버스(zCanvas) 사용**
- 차이는 노출되는 도구 세트:
  - 편집: 텍스트 관련 도구만 (텍스트 배치, 크기, 회전, 폰트, 기울임)
  - 상세 편집: 전체 이미지 편집 도구 (브러시, 인페인팅, 선택 등)
- feature flag로 모드별 도구 preset 구분
- embed 작업은 Agent 2의 bitmappery 커스터마이징 완료 후 진행
