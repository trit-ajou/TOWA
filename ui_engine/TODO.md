...
- 디자인 토큰(색상, 폰트 등)은 bitmappery의 기존 스타일과 일관성 유지 시도
- 더미 데이터로 UI만 구현하는 게 목표, 백엔드 연동 불필요

---

## Agent 2: bitmappery 커스터마이징 (상세 편집 뷰)

### 목표
bitmappery를 만화 번역 워크스테이션의 상세 편집 뷰에 맞게 커스터마이징. 불필요한 기능을 비활성화하고, 만화 번역에 필요한 UI 개선 사항을 파악.
...
1. **bitmappery 코드 분석 및 기능 분류**
   ...
   - 분류 결과를 Notion 설계 문서 DB에 기록 (분류: UI 엔진)

2. **불필요한 기능 비활성화**
   - 클라우드 연동 (Dropbox, Google Drive, S3) 비활성화
   - GIF 생성 기능 비활성화
   - PDF import 비활성화
   - bitmappery 자체 프로젝트 파일(.bpy) 저장/불러오기 — 추후 자체 포맷으로 대체 예정이므로 일단 유지
   - 비활성화는 코드 삭제가 아닌 off 방식 (주석, 조건부 렌더링, feature flag 등)

3. **UI 커스터마이징**
   - 불필요한 메뉴 항목 숨기기 (File 메뉴의 클라우드 관련 등)
   - 만화 번역에 자주 쓰는 도구를 쉽게 접근할 수 있도록 도구함 재배치 검토
   - 현재 UI의 문제점이나 개선 가능한 부분 목록화
