...
- 배경: 00:34의 v3 버전업 배포 후 사용자 계정에서 썸네일이 아예 로딩되지 않음. 서버 로그상 썸네일 요청 0건 = 요청 전 단계에서 정지. 원인: 옛 번들 탭이 v2 연결을 쥔 채 `versionchange`를 처리하지 않아 v3 업그레이드가 막히고, `getCacheDB()`가 무한 대기 → 모든 캐시 읽기(썸네일·페이지)가 멈춤. 재현: 옛 탭 연결 유지 시 썸네일 GET 0건
- `cache-db.ts`: (1) `blocked` 시 캐시를 사용 불가로 표시하고 대기 중이던 호출을 즉시 실패시켜 서버로 폴백(BlobCache는 기존대로 예외를 삼킴). (2) Chrome은 앞선 대기 요청 뒤에 줄 선 open에는 `blocked`를 다시 보내지 않아(전체 새로고침 시) 이벤트만으론 부족 → **3초 안에 열리지 않으면 blocked로 간주**. 뒤늦게 열리면 캐시 재사용. (3) `blocking` 시 스스로 연결을 닫아 향후 버전업을 막지 않음, `terminated`도 처리
- 검증: `cache-db-blocked.spec.ts`(옛 연결 유지 중 get/set이 멈추지 않음, 연결 해제 후 캐시 복귀, 상위 버전 open을 막지 않음 — 수정 전 두 테스트 모두 hang). vitest 13/13, `vue-tsc` 0. Playwright 두 탭 재현(옛 탭이 v2 유지): 수정 전 썸네일 GET 0 / 표시 0 → 수정본 GET 16 / 표시 16
